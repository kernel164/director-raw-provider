...
package com.cloudera.director.raw.compute;

import com.cloudera.director.raw.util.HostGroups;
 ...
 */
public class RawComputeProvider
    extends AbstractComputeProvider<RawComputeInstance, RawComputeInstanceTemplate> {

  private static final Logger LOG = Logger.getLogger(RawComputeProvider.class.getName());

  protected static final List<ConfigurationProperty> CONFIGURATION_PROPERTIES =
      ConfigurationPropertiesUtil.asConfigurationPropertyList(
          RawComputeProviderConfigurationPropertyToken.values());
  ...
  public RawComputeProvider(Configured configuration,
    ...
    String hostGroupExpressions = configuration.getConfigurationValue(
        RawComputeProviderConfigurationPropertyToken.HOSTS, localizationContext);
    ...
    this.resourceTemplateConfigurationValidator =
        new CompositeConfigurationValidator(METADATA.getResourceTemplateConfigurationValidator(),
            new RawComputeInstanceTemplateConfigurationValidator(this));
  }
  ...
  @Override
  public RawComputeInstanceTemplate createResourceTemplate(
      String name, Configured configuration, Map<String, String> tags) {
    return new RawComputeInstanceTemplate(name, configuration, tags, getLocalizationContext());
  }

  @Override
  public synchronized void allocate(RawComputeInstanceTemplate template,
  ...
  @Override
  public synchronized Collection<RawComputeInstance> find(
      RawComputeInstanceTemplate template, Collection<String> instanceIds)
      throws InterruptedException {

    List<RawComputeInstance> result = new ArrayList<RawComputeInstance>();
    for (String currentId : instanceIds) {
      String host = allocations.get(currentId);
      if (host != null) {
        try {
          result.add(new RawComputeInstance(template, currentId, InetAddress.getByName(host)));
  ...
  @Override
  public synchronized void delete(RawComputeInstanceTemplate template,
