...
public class RawLauncher extends AbstractLauncher {

  public RawLauncher() {
    super(Collections.singletonList(RawCloudProvider.METADATA), null);
  }

  @Override
  public CloudProvider createCloudProvider(String cloudProviderId, Configured ignored, Locale locale) {
    if (!RawCloudProvider.ID.equals(cloudProviderId)) {
      throw new NoSuchElementException("Cloud provider not found: " + cloudProviderId);
    }

    return new RawCloudProvider(getLocalizationContext(locale));
  }
}
